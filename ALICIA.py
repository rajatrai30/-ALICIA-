...
if __name__ == '__main__':
    ...
    image1 = Image.open('extrafiles/images/gifloader.gif')
    ...
    # welcome label
    welcLbl = Label(root1, text='Hello User!!!,\nWelcome to the Security Section \n of \n "ALICIA"', font=(
        'Arial Bold', 15), fg='#0f0931', bg=background)
    welcLbl.pack(padx=10, pady=20)

    # add face
    loginStatus = Label(root1, text='LOCKED', font=(
        'Arial Bold', 15), bg=background, fg='red')
    loginStatus.pack(pady=(40, 20))

    if os.path.exists('userData/trainer.yml') == False:
        loginStatus['text'] = 'Your Face is not registered with us'
        addFace = Button(root1, text='   Register Face   ', font=(
            'Arial', 12), bg='#0f0931', fg='white', relief=FLAT, command=lambda: raise_frame(root2))
    ...
    # status of add face
    faceStatus = Label(root1, text='(Face Not Detected)',
                       font=('Arial 10'), fg=textColor, bg=background)
    faceStatus.pack(pady=5)

    ########  ADDING THE FACE SCREEN  #######

    image2 = Image.open('extrafiles/images/alicia1.jpg')
    ...
    userFrame2 = Frame(detailFrame2, bd=10, width=300,
                       height=250, relief=FLAT, bg=background)
    userFrame2.pack(padx=10, pady=10)

    # progress
    progress_bar = ttk.Progressbar(
        root2, orient=HORIZONTAL, length=303, mode='determinate')
    
    # name
    nameLbl = Label(userFrame2, text='Name', font=(
        'Arial Bold', 12), fg='#303E54', bg=background)
    nameLbl.place(x=10, y=10)
    nameField = Entry(userFrame2, bd=5, font=(
        'Arial Bold', 10), width=25, relief=FLAT, bg='#D4D5D7')
    nameField.focus()
    nameField.place(x=80, y=10)

    genLbl = Label(userFrame2, text='Gender', font=(
        'Arial Bold', 12), fg='#303E54', bg=background)
    genLbl.place(x=10, y=50)
    r = IntVar()
    s = ttk.Style()
    s.configure('Wild.TRadiobutton', background=background, foreground=textColor, font=(
        'Arial Bold', 10), focuscolor=s.configure(".")["background"])
    genMale = ttk.Radiobutton(userFrame2, text='Male', value=1,
                              variable=r, style='Wild.TRadiobutton', takefocus=False)
    genMale.place(x=80, y=52)
    genFemale = ttk.Radiobutton(userFrame2, text='Female', value=2,
                                variable=r, style='Wild.TRadiobutton', takefocus=False)
    ...
    agr = IntVar()
    sc = ttk.Style()
    sc.configure('Wild.TCheckbutton', background=background, foreground='#303E54', font=(
        'Arial Bold', 10), focuscolor=sc.configure(".")["background"])
    ...
    # add face
    addBtn = Button(userFrame2, text='    Add Face    ', font=(
        'Arial Bold', 12), bg='#0f0931', fg='white', command=Add_Face, relief=FLAT)
    addBtn.place(x=90, y=150)

    # status of add face
    statusLbl = Label(userFrame2, text='', font=(
        'Arial 10'), fg=textColor, bg=background)
    ...
    Label(root3, text="Choose Your Virtual Face ID", font=(
        'arial', 15), bg=background, fg='#303E54').pack()
    ...
    avtb1 = Button(avatarContainer, image=avtr1, bg=background,
                   activebackground=background, relief=FLAT, bd=0, command=lambda: selectAVATAR(1))
    avtb1.grid(row=0, column=0, ipadx=25, ipady=10)

    avtb2 = Button(avatarContainer, image=avtr2, bg=background,
                   activebackground=background, relief=FLAT, bd=0, command=lambda: selectAVATAR(2))
    avtb2.grid(row=0, column=1, ipadx=25, ipady=10)

  

    Button(root3, text='         Submit         ', font=('Arial Bold', 15), bg='#0f0931',
           fg='white', bd=0, relief=FLAT, command=SuccessfullyRegistered).pack()
    ...
    usernameLbl = Label(root4, text="RAJAT RAI", font=(
        'Arial Bold', 15), bg=background, fg='#85AD4F')
    usernameLbl.pack(pady=(0, 70))

    Label(root4, text="Your account has been successfully activated!", font=(
        'Arial Bold', 15), bg=background, fg='#303E54', wraplength=300).pack(pady=10)
    Label(root4, text="Launch the ALICIA Security APP again to get started the conversation with your Virtual Assistant",
          font=('Arial Bold', 13), bg=background, fg='#A3A5AB', wraplength=350).pack()

    Button(root4, text='     OK     ', bg='#0475BB', fg='white', font=(
        'Arial Bold', 18), bd=0, relief=FLAT, command=lambda: quit()).pack(pady=50)
